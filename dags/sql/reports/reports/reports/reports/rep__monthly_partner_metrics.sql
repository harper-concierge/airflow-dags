{% if is_modified %}
DROP MATERIALIZED VIEW IF EXISTS {{ schema }}.rep__monthly_partner_metrics CASCADE;
{% endif %}
CREATE MATERIALIZED VIEW IF NOT EXISTS {{ schema }}.rep__monthly_partner_metrics AS
WITH
...
all_orders AS (
    ...
    GROUP BY
        dt.dim_yearmonth_sc,
        o.partner_name
    ORDER BY dt.dim_yearmonth_sc
),
combined_data AS (
    SELECT
        o.metric_date AS dim_yearmonth_sc,
        o.partner_name,
...
WITH NO DATA;
{% if is_modified %}
CREATE UNIQUE INDEX IF NOT EXISTS rep__monthly_partner_metrics_idx ON {{ schema }}.rep__monthly_partner_metrics (dim_yearmonth_sc, partner_name);
{% endif %}
REFRESH MATERIALIZED VIEW {{ schema }}.rep__monthly_partner_metrics;
