...
all_orders AS (
    ...
    GROUP BY
        createdat__dim_yearmonth_sc
    ORDER BY createdat__dim_yearmonth_sc
),
combined_data AS (
    SELECT
...
WITH NO DATA;
{% if is_modified %}
CREATE UNIQUE INDEX IF NOT EXISTS rep__monthly_total_metrics_idx ON {{ schema }}.rep__monthly_total_metrics (dim_yearmonth_sc);
{% endif %}
